...
async def collect_single_device_port(ip: str, community: str, sys_type: str):
    """采集单个设备的端口状态"""
    limiter = get_concurrency_limiter()
    async with limiter:
        try:
            port = PortInfo(ip, community, sys_type)
            port_data = await port.getPorts()
            return {
                'ip': ip,
                'ports': port_data
            }
        except Exception as e:
            logger.error(f"采集设备 {ip} 端口状态失败: {e}")
            return None
...
async def collect_and_save_batch(device_batch, batch_id, use_queue=True):
    ...
    for device in device_batch:
        ...
        tasks.append(collect_single_device_port(ip, community, sys_type))
...
async def run():
    ...
    try:
        ...
        logger.info(f"端口状态采集完成 - 成功: {total_success}, 失败: {total_failed}, 耗时: {elapsed:.2f}秒")

    except Exception as e:
        logger.error(f"端口状态采集任务执行失败: {e}")
