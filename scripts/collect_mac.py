...
async def collect_and_save_batch(device_batch, batch_id, use_queue=True):
    ...
    for device in device_batch:
        ...
        tasks.append(collect_single_device_mac(ip, community, sys_type))
...
async def run():
    ...
    try:
        ...
        logger.info(f"MAC地址表采集完成 - 成功: {total_success}, 失败: {total_failed}, 耗时: {elapsed:.2f}秒")

    except Exception as e:
        logger.error(f"MAC地址表采集任务执行失败: {e}")
    finally:
        # 停止队列并等待所有数据写入完成
