...
async def collect_single_device_lldp(ip: str, community: str, sys_type: str):
    """采集单个设备的LLDP邻居信息"""
    limiter = get_concurrency_limiter()
    async with limiter:
        try:
            lldp = LLDPInfo(ip, community, sys_type)
            lldp_data = await lldp.getLLDPInfos()
            return {
                'ip': ip,
                'lldp': lldp_data
            }
        except Exception as e:
            logger.error(f"采集设备 {ip} LLDP信息失败: {e}")
            return None
...
async def collect_and_save_batch(device_batch, batch_id, use_queue=True):
    ...
    for device in device_batch:
        ...
        tasks.append(collect_single_device_lldp(ip, community, sys_type))
...
async def run():
    """
    主执行函数 - 采集所有设备的LLDP邻居信息
    分批采集和保存，避免等待时间过长
    """
    logger.info("开始执行LLDP邻居信息采集任务")
    ...
    try:
        ...
        logger.info(f"LLDP邻居信息采集完成 - 成功: {total_success}, 失败: {total_failed}, 耗时: {elapsed:.2f}秒")

    except Exception as e:
        logger.error(f"LLDP邻居信息采集任务执行失败: {e}")
