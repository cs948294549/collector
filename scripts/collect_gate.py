...
async def collect_single_device_gate(ip: str, community: str, sys_type: str):
    """采集单个设备的IPv4网关信息"""
    try:
        ...
        logger.error(f"采集设备 {ip} IPv4网关信息失败: {e}")
        return None
...
async def collect_and_save_batch(device_batch, batch_id):
    ...
    for device in device_batch:
        ...
        tasks.append(collect_single_device_gate(ip, community, sys_type))
...
async def run():
    ...
    try:
        ...
        logger.info(f"IPv4网关信息采集完成 - 成功: {total_success}, 失败: {total_failed}, 耗时: {elapsed:.2f}秒")
