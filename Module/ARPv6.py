...
class ARPv6Table(object):
    '''
    普通设备采集节点：
    arp = 1.3.6.1.2.1.55.1.12.1.2
    索引为最后5位 = 接口ID+IP地址
    '''
    ...
    async def __getDefaultArps(self):
        ...
        '''
        oid_nodes = {
            "arp_mac": "1.3.6.1.2.1.55.1.12.1.2",
        }
        # header = 80 byte
        ...
        try:
            ...
            for primary_key in oid_cache.keys():
                ...
                _ipv6 = "".join([str(hex(int(_ii)))[2:].upper().zfill(2) for _ii in pri_keys[1:]])
                oid_cache[primary_key]["arp_ip"] = ":".join([_ipv6[i:i + 4] for i in range(0, len(_ipv6), 4)])
...
if __name__ == '__main__':
    ...
    async def test():
        a = ARPv6Table("10.163.87.92", "Mrtg.Netease")
        res = await a.getARPs()
        print(len(res))
        for i in res:
            print(i)
